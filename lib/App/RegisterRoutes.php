<?php

/**
 * MultiFlexi API
 * PHP version 8.1.
 *
 * @author  OpenAPI Generator team
 *
 * @see    https://github.com/openapitools/openapi-generator
 */

/**
 * MultiFlexi API provides comprehensive access to job execution, application management, event-driven processing, and user data operations. The API includes GDPR compliance endpoints for data export (Article 15 - Right of Access) and uses OAuth2 Application Flow for security.
 * The version of the OpenAPI document: 1.4.0
 * Contact: vitezslav.dvorak@spojenet.cz
 * Generated by: https://github.com/openapitools/openapi-generator.git.
 */

declare(strict_types=1);

/**
 * This file is part of the MultiFlexi package
 *
 * https://multiflexi.eu/
 *
 * (c) Vítězslav Dvořák <http://vitexsoftware.com>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

namespace MultiFlexi\Api\App;

use Dyorg\TokenAuthentication;
use MultiFlexi\Api\Auth\BasicAuthenticator;
use Psr\Http\Message\ResponseInterface;
use Psr\Http\Message\ServerRequestInterface;
use Slim\Exception\HttpNotImplementedException;

/**
 * RegisterRoutes Class Doc Comment.
 *
 * @author  OpenAPI Generator team
 *
 * @see    https://github.com/openapitools/openapi-generator
 */
class RegisterRoutes
{
    /**
     * @var array[] list of all api operations
     */
    private array $operations = [
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/app/',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractAppApi',
            'userClassname' => 'AppApi',
            'operationId' => 'setAppById',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "application created or updated",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/App"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "App not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/app/{appId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractAppApi',
            'userClassname' => 'AppApi',
            'operationId' => 'getAppById',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/App"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "App not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/apps.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractAppApi',
            'userClassname' => 'AppApi',
            'operationId' => 'listApps',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "object",
        "additionalProperties" : {
          "$ref" : "#/components/schemas/App"
        },
        "description" : "Apps keyed by their id"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid status value"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/auditlog/{auditLogId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractAuditlogApi',
            'userClassname' => 'AuditlogApi',
            'operationId' => 'getAuditLogById',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/AuditLogEntry"
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Audit Log entry not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/auditlog.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractAuditlogApi',
            'userClassname' => 'AuditlogApi',
            'operationId' => 'listAuditLog',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/AuditLogEntry"
        }
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/company/',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractCompanyApi',
            'userClassname' => 'CompanyApi',
            'operationId' => 'setCompanyById',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Company created or updated",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/Company"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Company not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/company/{companyId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractCompanyApi',
            'userClassname' => 'CompanyApi',
            'operationId' => 'getCompanyById',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/Company"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Company not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/companies.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractCompanyApi',
            'userClassname' => 'CompanyApi',
            'operationId' => 'listCompanies',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/Company"
        }
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid status value"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/credentials.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractCredentialApi',
            'userClassname' => 'CredentialApi',
            'operationId' => 'getAllUserCredentials',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/Credential"
        }
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/credential/{credentialId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractCredentialApi',
            'userClassname' => 'CredentialApi',
            'operationId' => 'getCredential',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/Credential"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid token supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/credential/{credentialId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractCredentialApi',
            'userClassname' => 'CredentialApi',
            'operationId' => 'updateCredentials',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Record was updated",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/updateCredentials_201_response"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid token supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/credential_types.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractCredentialTypeApi',
            'userClassname' => 'CredentialTypeApi',
            'operationId' => 'getAllCredentialTypes',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/CredentialType"
        }
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/credential_type/{credentialTypeID}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractCredentialTypeApi',
            'userClassname' => 'CredentialTypeApi',
            'operationId' => 'getCredentialType',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/getCredentialType_200_response"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/credential_type/{credentialTypeID}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractCredentialTypeApi',
            'userClassname' => 'CredentialTypeApi',
            'operationId' => 'updateCredentialType',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Record was updated",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/getCredentialType_200_response"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/logout',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractDefaultApi',
            'userClassname' => 'DefaultApi',
            'operationId' => 'logoutPost',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Odhlášení proběhlo úspěšně"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Uživatel není přihlášen nebo token je neplatný"
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractDefaultApi',
            'userClassname' => 'DefaultApi',
            'operationId' => 'rootGet',
            'responses' => [
                '301' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "redirect to index.html"
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/index.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractDefaultApi',
            'userClassname' => 'DefaultApi',
            'operationId' => 'getApiIndex',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "OK"
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/jobs/status.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractDefaultApi',
            'userClassname' => 'DefaultApi',
            'operationId' => 'getJobsStatus',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/JobsStatus"
        }
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/login.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractDefaultApi',
            'userClassname' => 'DefaultApi',
            'operationId' => 'loginSuffixGet',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "OK"
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/login.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractDefaultApi',
            'userClassname' => 'DefaultApi',
            'operationId' => 'loginSuffixPost',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "OK"
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/ping.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractDefaultApi',
            'userClassname' => 'DefaultApi',
            'operationId' => 'pingSuffixGet',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "OK"
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/status.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractDefaultApi',
            'userClassname' => 'DefaultApi',
            'operationId' => 'statusSuffixGet',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "OK",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/Status"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/eventrule/',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractEventruleApi',
            'userClassname' => 'EventruleApi',
            'operationId' => 'setEventRuleById',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "EventRule created or updated",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/EventRule"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid data supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'DELETE',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/eventrule/{eventRuleId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractEventruleApi',
            'userClassname' => 'EventruleApi',
            'operationId' => 'deleteEventRuleById',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "EventRule deleted"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "EventRule not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/eventrule/{eventRuleId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractEventruleApi',
            'userClassname' => 'EventruleApi',
            'operationId' => 'getEventRuleById',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/EventRule"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "EventRule not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/eventrules.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractEventruleApi',
            'userClassname' => 'EventruleApi',
            'operationId' => 'listEventRules',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/EventRule"
        }
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid status value"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/eventsource/',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractEventsourceApi',
            'userClassname' => 'EventsourceApi',
            'operationId' => 'setEventSourceById',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "EventSource created or updated",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/EventSource"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid data supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'DELETE',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/eventsource/{eventSourceId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractEventsourceApi',
            'userClassname' => 'EventsourceApi',
            'operationId' => 'deleteEventSourceById',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "EventSource deleted"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "EventSource not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/eventsource/{eventSourceId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractEventsourceApi',
            'userClassname' => 'EventsourceApi',
            'operationId' => 'getEventSourceById',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/EventSource"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "EventSource not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/eventsources.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractEventsourceApi',
            'userClassname' => 'EventsourceApi',
            'operationId' => 'listEventSources',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/EventSource"
        }
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid status value"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/eventsource/{eventSourceId}/test.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractEventsourceApi',
            'userClassname' => 'EventsourceApi',
            'operationId' => 'testEventSourceConnection',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Connection test result",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/testEventSourceConnection_200_response"
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "EventSource not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/data-export-download',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractGdprApi',
            'userClassname' => 'GdprApi',
            'operationId' => 'downloadDataExport',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Data export file download",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/DataExportData"
      },
      "example" : {
        "export_metadata" : {
          "export_date" : "2025-10-20T16:15:00Z",
          "user_id" : 42,
          "gdpr_article" : "Article 15 - Right of Access",
          "data_controller" : "MultiFlexi"
        },
        "user_profile" : {
          "username" : "john.doe",
          "email" : "john.doe@example.com",
          "first_name" : "John",
          "last_name" : "Doe"
        }
      }
    },
    "text/plain" : {
      "schema" : {
        "type" : "string",
        "example" : "MultiFlexi - Personal Data Export\nGenerated: 2025-10-20T16:15:00Z\n\nUSER PROFILE\n------------\nUsername: john.doe\nEmail: john.doe@example.com"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid or missing token"
}
EOD,
                ],
                '403' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Token expired, already used, or access denied"
}
EOD,
                ],
                '500' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Failed to generate export"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/data-export',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractGdprApi',
            'userClassname' => 'GdprApi',
            'operationId' => 'requestDataExport',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Export request successful",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/requestDataExport_200_response"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid request parameters",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/ErrorResponse"
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '403' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Access denied or suspicious activity detected",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/ErrorResponse"
      }
    }
  }
}
EOD,
                ],
                '429' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Rate limit exceeded",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/ErrorResponse"
      }
    }
  }
}
EOD,
                ],
                '500' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Internal server error",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/ErrorResponse"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/data-export',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractGdprApi',
            'userClassname' => 'GdprApi',
            'operationId' => 'requestDataExportPost',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Export request successful",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/requestDataExport_200_response"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid request parameters",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/ErrorResponse"
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '403' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Access denied or suspicious activity detected",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/ErrorResponse"
      }
    }
  }
}
EOD,
                ],
                '429' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Rate limit exceeded",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/ErrorResponse"
      }
    }
  }
}
EOD,
                ],
                '500' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Internal server error",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/ErrorResponse"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/job/',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractJobApi',
            'userClassname' => 'JobApi',
            'operationId' => 'setjobById',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Job scheduled",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/Job"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid request body"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "RunTemplate not found"
}
EOD,
                ],
                '409' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "RunTemplate is not active"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/job/{jobId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractJobApi',
            'userClassname' => 'JobApi',
            'operationId' => 'getjobById',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/Job"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "App not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/jobs.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractJobApi',
            'userClassname' => 'JobApi',
            'operationId' => 'listjobs',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "object",
        "additionalProperties" : {
          "$ref" : "#/components/schemas/Job"
        },
        "description" : "Jobs keyed by their id"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid status value"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/runtemplate',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractRuntemplateApi',
            'userClassname' => 'RuntemplateApi',
            'operationId' => 'setRunTemplateById',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "RunTemplate created or updated",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/RunTemplate"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "RunTemplate not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/runtemplate/{runTemplateId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractRuntemplateApi',
            'userClassname' => 'RuntemplateApi',
            'operationId' => 'getRunTemplateById',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/RunTemplate"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "RunTemplate not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/runtemplates.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractRuntemplateApi',
            'userClassname' => 'RuntemplateApi',
            'operationId' => 'listRunTemplates',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "object",
        "additionalProperties" : {
          "$ref" : "#/components/schemas/RunTemplate"
        },
        "description" : "Run templates keyed by their id"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid status value"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/runtemplate/{runTemplateId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractRuntemplateApi',
            'userClassname' => 'RuntemplateApi',
            'operationId' => 'updateRunTemplateById',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "RunTemplate updated successfully",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/RunTemplate"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied or ID mismatch",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/updateRunTemplateById_400_response"
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "RunTemplate not found",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/updateRunTemplateById_404_response"
      }
    }
  }
}
EOD,
                ],
                '500' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Update failed",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/updateRunTemplateById_500_response"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/task/{taskId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractTaskApi',
            'userClassname' => 'TaskApi',
            'operationId' => 'getTaskById',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/Task"
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Task not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/tasks.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractTaskApi',
            'userClassname' => 'TaskApi',
            'operationId' => 'listTasks',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/Task"
        }
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/topics.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractTopicApi',
            'userClassname' => 'TopicApi',
            'operationId' => 'getAllTopics',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/Topic"
        }
      }
    }
  }
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/topic/{topicId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractTopicApi',
            'userClassname' => 'TopicApi',
            'operationId' => 'getTopic',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/getTopic_200_response"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/topic/{topicId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractTopicApi',
            'userClassname' => 'TopicApi',
            'operationId' => 'updateTopic',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Record was updated",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/getTopic_200_response"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/user/',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractUserApi',
            'userClassname' => 'UserApi',
            'operationId' => 'setUserById',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "User created or updated",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/User"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "User not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/user/{userId}.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractUserApi',
            'userClassname' => 'UserApi',
            'operationId' => 'getUserById',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/User"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "User not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/users.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractUserApi',
            'userClassname' => 'UserApi',
            'operationId' => 'listUsers',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/User"
        }
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid status value"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/company/{companyId}/user/',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractUserCompanyApi',
            'userClassname' => 'UserCompanyApi',
            'operationId' => 'assignUserToCompany',
            'responses' => [
                '201' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "User assigned to company",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/CompanyUser"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid request"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Company or user not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/company/{companyId}/users.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractUserCompanyApi',
            'userClassname' => 'UserCompanyApi',
            'operationId' => 'listCompanyUsers',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/listCompanyUsers_200_response_inner"
        }
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Company not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'DELETE',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/company/{companyId}/user/{userId}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractUserCompanyApi',
            'userClassname' => 'UserCompanyApi',
            'operationId' => 'unassignUserFromCompany',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "User unassigned (or was not assigned)",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/unassignUserFromCompany_200_response"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Company or user not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'GET',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/user/{userId}/roles.{suffix}',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractUserRoleApi',
            'userClassname' => 'UserRoleApi',
            'operationId' => 'getUserRoles',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Successful operation",
  "content" : {
    "application/json" : {
      "schema" : {
        "type" : "array",
        "items" : {
          "$ref" : "#/components/schemas/RbacRole"
        }
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid ID supplied"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "User not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
        [
            'httpMethod' => 'POST',
            'basePathWithoutHost' => '/api/VitexSoftware/MultiFlexi/1.0.0',
            'path' => '/user/{userId}/roles/',
            'apiPackage' => 'MultiFlexi\Api\Server',
            'classname' => 'AbstractUserRoleApi',
            'userClassname' => 'UserRoleApi',
            'operationId' => 'setUserRoles',
            'responses' => [
                '200' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Roles updated",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/setUserRoles_200_response"
      }
    }
  }
}
EOD,
                ],
                '400' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Invalid request or unknown role names"
}
EOD,
                ],
                '401' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "Authentication information is missing or invalid",
  "headers" : {
    "WWW_Authenticate" : {
      "style" : "simple",
      "explode" : false,
      "schema" : {
        "type" : "string"
      }
    }
  }
}
EOD,
                ],
                '404' => [
                    'jsonSchema' => <<<'EOD'
{
  "description" : "User not found"
}
EOD,
                ],
            ],
            'authMethods' => [
                // http security schema named 'basicAuth'
                [
                    'type' => 'http',
                    'isBasic' => true,
                    'isBearer' => false,
                    'isApiKey' => false,
                    'isOAuth' => false,
                ],
            ],
        ],
    ];

    /**
     * Add routes to Slim app.
     *
     * @param \Slim\App $app Pre-configured Slim application instance
     *
     * @throws HttpNotImplementedException When implementation class doesn't exists
     */
    public function __invoke(\Slim\App $app): void
    {
        $app->options('/{routes:.*}', static function (ServerRequestInterface $request, ResponseInterface $response) {
            // CORS Pre-Flight OPTIONS Request Handler
            return $response;
        });

        // create mock middleware factory
        /** @var \Psr\Container\ContainerInterface */
        $container = $app->getContainer();
        /** @var null|\OpenAPIServer\Mock\OpenApiDataMockerRouteMiddlewareFactory */
        $mockMiddlewareFactory = null;

        if ($container->has(\OpenAPIServer\Mock\OpenApiDataMockerRouteMiddlewareFactory::class)) {
            // I know, anti-pattern. Don't retrieve dependency directly from container
            $mockMiddlewareFactory = $container->get(\OpenAPIServer\Mock\OpenApiDataMockerRouteMiddlewareFactory::class);
        }

        foreach ($this->operations as $operation) {
            $callback = static function (ServerRequestInterface $request) use ($operation): void {
                $message = "How about extending {$operation['classname']} by {$operation['apiPackage']}\\{$operation['userClassname']} class implementing {$operation['operationId']} as a {$operation['httpMethod']} method?";

                throw new HttpNotImplementedException($request, $message);
            };
            $middlewares = [];

            if (class_exists("\\{$operation['apiPackage']}\\{$operation['userClassname']}")) {
                // Notice how we register the controller using the class name?
                // PHP-DI will instantiate the class for us only when it's actually necessary
                $callback = ["\\{$operation['apiPackage']}\\{$operation['userClassname']}", $operation['operationId']];
            }

            if ($mockMiddlewareFactory) {
                $mockSchemaResponses = array_map(static function ($item) {
                    return json_decode($item['jsonSchema'], true);
                }, $operation['responses']);
                $middlewares[] = $mockMiddlewareFactory->create($mockSchemaResponses);
            }

            $route = $app->map(
                [$operation['httpMethod']],
                "{$operation['basePathWithoutHost']}{$operation['path']}",
                $callback,
            )->setName($operation['operationId']);

            // Add authentication middleware based on the operation's authMethods
            if ($operation['authMethods']) {
                foreach ($operation['authMethods'] as $authMethod) {
                    if ($authMethod['isBasic']) {
                        $route->add(new TokenAuthentication([
                            'path' => '/',
                            'authenticator' => new BasicAuthenticator(),
                            'regex' => '/Basic\s+(.*)$/i',
                            'header' => 'Authorization',
                            'parameter' => null,
                            'cookie' => null,
                            'argument' => null,
                            'attribute' => 'authorization_token',
                            'error' => ['MultiFlexi\Api\Auth\BasicAuthenticator', 'handleUnauthorized'],
                        ]));
                    }
                }
            }

            foreach ($middlewares as $middleware) {
                $route->add($middleware);
            }
        }
    }
}
